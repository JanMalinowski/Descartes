...
class XGBEnsemble:
    ...
    def train(self, data: pd.DataFrame, params: dict, n_splits: int = 5) -> np.array:
        ...
        for k in range(n_splits):
            ...
            dtrain = xgb.DMatrix(
                data.loc[train_idxs, cols], label=data.loc[train_idxs, "target"]
            )
            dval = xgb.DMatrix(
                data.loc[val_idxs, cols], label=data.loc[val_idxs, "target"]
            )
    ...
    def predict(self, data: pd.DataFrame) -> np.array:
        ...
        for i, model in enumerate(self.models):
            ...
            preds += model.predict(
                dtest, ntree_limit=model.best_ntree_limit + 50
            ) / len(self.models)
        return preds

    def save(self, path: Path) -> None:
        for i, model in enumerate(self.models):
            joblib.dump(model, os.path.join(path, f"xgb_model{i}.pkl"))

    def load(self, path: Path) -> None:
        models = os.listdir(path)
        for i, model in enumerate(models):
            self.models.append(joblib.load(os.path.join(path, model)))
